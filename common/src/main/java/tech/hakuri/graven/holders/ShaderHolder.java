...
public class ShaderHolder {
    ...
    public void processEntityOutlineTarget(RenderTarget target, Shader shader) {
        ...
        renderPass("graven_shader_effect", target, shaderSwap, pipeline(shader), shaderUniforms, usesColors(shader));
        // This fullscreen copy pass measured higher FPS than CommandEncoder.copyTextureToTexture on the tested renderer.
        renderPass("graven_shader_copy", shaderSwap, target, copyPipeline, null, false);
    }
    ...
    private void ensureSwap(int width, int height) {
        if (shaderSwap == null) {
            shaderSwap = new TextureTarget("Graven Shader Swap", width, height, false);
        }
    ...
    private void ensureHandTarget(int width, int height) {
        if (handTarget == null) {
            handTarget = new TextureTarget("Graven Shader Hands", width, height, true);
        }
    ...
    private void ensureChestTarget(int width, int height) {
        if (chestTarget == null) {
            chestTarget = new TextureTarget("Graven Shader Chests", width, height, true);
        }
