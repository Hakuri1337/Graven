...
import tech.hakuri.graven.modules.impl.render.CameraClip;
...
public class MixinCamera {
    ...
    @Unique
    private final Pose[] graven$lastPoses = new Pose[2];

    @Unique
    private boolean graven$isStandingCrouchingTransition() {
        return graven$lastPoses[1] == Pose.STANDING && graven$lastPoses[0] == Pose.CROUCHING || graven$lastPoses[1] == Pose.CROUCHING && graven$lastPoses[0] == Pose.STANDING;
    }
    ...
    private void hookSneakTweakCameraTick(CallbackInfo ci) {
        ...
        if (pose != graven$lastPoses[0]) {
            graven$lastPoses[1] = graven$lastPoses[0];
            graven$lastPoses[0] = pose;
        }

        if (SneakTweak.INSTANCE.shouldSnapCameraEyeHeight() && graven$isStandingCrouchingTransition()) {
    ...
    private float hookSneakTweakCameraSpeed(float modifier) {
        if (graven$isStandingCrouchingTransition()) {
