...
import tech.hakuri.graven.elements.impl.notification.NotificationMode;
...
public class PlayerAlarms extends Module {
        ...
        if (currentConnection == null) {
            ...
            for (var entry : mc.getConnection().getListedOnlinePlayers()) {
                ...
                if (shouldAlarm(playerName)) {
                    ...
                    sendAlert(joinChatMessage.getValue(), GravenTranslations.PlayerAlarms.JOIN_ALERT_TEXT, playerName, id, ChatFormatting.RED);
        ...
        for (UUID id : currentInRender) {
            if (!playersInRender.contains(id)) {
                ...
                if (playerName != null && shouldAlarm(playerName)) {
                    ...
                    sendAlert(enterRDChatMessage.getValue(), GravenTranslations.PlayerAlarms.ENTER_RD_ALERT_TEXT, playerName, id, ChatFormatting.DARK_RED);
                }
            }
        }
        ...
        for (UUID id : playersInRender) {
            if (!currentInRender.contains(id)) {
                ...
                if (playerName != null && shouldAlarm(playerName)) {
                    ...
                    sendAlert(leaveRDChatMessage.getValue(), GravenTranslations.PlayerAlarms.LEAVE_RD_ALERT_TEXT, playerName, id, ChatFormatting.DARK_GREEN);
                }
            }
        }
            ...
            if (packet.actions().contains(Action.ADD_PLAYER)) {
                for (Entry entry : packet.entries()) {
                    ...
                    if (!alarmedJoinPlayers.contains(entry.profileId()) && shouldAlarm(playerName)) {
                        startRing(joinRing, joinRings.getValue(), joinRingDelay.getValue());
                        sendAlert(joinChatMessage.getValue(), GravenTranslations.PlayerAlarms.JOIN_ALERT_TEXT, playerName, entry.profileId(), ChatFormatting.RED);
                    ...
                    if (oldMode != newMode) {
                        ...
                        if (playerName != null && shouldAlarm(playerName)) {
                            ...
                            if (gamemodeChatMessage.getValue()) {
                                String msg = GravenTranslations.PlayerAlarms.GAMEMODE_ALERT_TEXT.getTranslatedName()
        ...
        if (event.getPacket() instanceof ClientboundPlayerInfoRemovePacket removePacket) {
            for (UUID id : removePacket.profileIds()) {
                ...
                if (shouldAlarm(playerName)) {
                    ...
                    sendAlert(leaveChatMessage.getValue(), GravenTranslations.PlayerAlarms.LEAVE_ALERT_TEXT, playerName, id, ChatFormatting.GREEN);
                }
    ...
    private String getGamemodeName(UUID id) {
        ...
        return GravenTranslations.PlayerAlarms.UNKNOWN_GAMEMODE.getTranslatedName();
    }

    private static String translateGamemode(GameType gameType) {
        if (gameType == GameType.SURVIVAL)
            return GravenTranslations.PlayerAlarms.GAMEMODE_SURVIVAL.getTranslatedName();
        if (gameType == GameType.CREATIVE)
            return GravenTranslations.PlayerAlarms.GAMEMODE_CREATIVE.getTranslatedName();
        if (gameType == GameType.ADVENTURE)
            return GravenTranslations.PlayerAlarms.GAMEMODE_ADVENTURE.getTranslatedName();
        if (gameType == GameType.SPECTATOR)
            return GravenTranslations.PlayerAlarms.GAMEMODE_SPECTATOR.getTranslatedName();
