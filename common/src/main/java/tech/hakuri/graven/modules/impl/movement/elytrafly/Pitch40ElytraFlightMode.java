...
public class Pitch40ElytraFlightMode extends ElytraFlightMode {
    ...
    private void initializeInWorld() {
        ...
        if (mc.player.getY() - 40.0 < elytraFly.pitch40lowerBounds.getValue()) {
            fail(GravenTranslations.ElytraFly.PITCH40_TOO_CLOSE_TO_LOWER_BOUNDS);
        }
    }
    ...
    private void updateTakeoff() {
        ...
        if (!canGlide(elytra.found())) {
            fail(GravenTranslations.ElytraFly.PITCH40_NO_USABLE_ELYTRA);
            return;
        }
    ...
    private void finishTakeoff() {
        ...
        pitch = DESCEND_PITCH;
        maintainFallFlying();
        Managers.NOTIFICATION.success(elytraFly.getTranslatedName(), GravenTranslations.ElytraFly.PITCH40_TAKEOFF_COMPLETE.getTranslatedName());
    }
