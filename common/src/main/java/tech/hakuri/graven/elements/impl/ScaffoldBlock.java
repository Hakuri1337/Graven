...
public class ScaffoldBlock extends HudModule {
    ...
    private Layout createLayout() {
        ...
        float labelWidth = textWidth(LABEL, labelScale, "graven-default");
    ...
    private void drawText(UiTree.Scope scope, Layout layout, AnimationState animation) {
        ...
        float numberY = this.y + (layout.height() - textHeight(layout.numberScale(), "graven-default")) / 2.0f;
        ...
        Consumer<UiTree.Scope> content = clipped -> {
            ...
            float labelY = this.y + (layout.height() - textHeight(layout.labelScale(), "graven-default")) / 2.0f;
    ...
    private float getNumberColumnWidth(float numberScale) {
        float width = Math.max(
                textWidth(previousCountText, numberScale, "graven-default"),
                textWidth(targetCountText, numberScale, "graven-default")
        );
    ...
    private void drawRollingNumber(UiTree.Scope scope, float numberScale, float columnX, float columnWidth, float textY, float alphaMul) {
        ...
        float contentWidth = Math.max(
                textWidth(previous, numberScale, "graven-default"),
                textWidth(target, numberScale, "graven-default")
        );
        ...
        for (int i = 0; i < maxLen; i++) {
            ...
            if (previousChar != '\0') {
                slotWidth = Math.max(slotWidth, textWidth(String.valueOf(previousChar), numberScale, "graven-default"));
            }
            if (targetChar != '\0') {
                slotWidth = Math.max(slotWidth, textWidth(String.valueOf(targetChar), numberScale, "graven-default"));
            }
