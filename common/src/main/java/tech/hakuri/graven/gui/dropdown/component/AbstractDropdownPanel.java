...
public abstract class AbstractDropdownPanel implements DropdownPanel {
    ...
    protected AbstractDropdownPanel(String id, TitleSupplier titleSupplier, String icon, int panelIndex) {
        ...
        this.icon = icon;
        this.introAnim = new Animation(Easing.EASE_OUT_SINE,
                DropdownTheme.isOpal() ? 100L + panelIndex * 80L : 120L + panelIndex * 45L);
    }
    ...
    public void drawBackground(UiTree.Scope scope, UiTextMetrics textMetrics) {
        ...
        if (DropdownTheme.isOpal()) {
            scope.outline(x, y, width, panelHeight, DropdownTheme.PANEL_RADIUS, 0.65f,
                    MD3Theme.withAlpha(MD3Theme.OUTLINE, 105));
            scope.rect(x + 4.0f, y + DropdownTheme.PANEL_HEADER_HEIGHT - 0.6f,
                    width - 8.0f, 0.6f, MD3Theme.withAlpha(MD3Theme.OUTLINE, 52));
        }

        float iconX = x + 7.5f;
        float textX = icon == null || icon.isBlank() ? x + 10.0f : iconX + 16.0f;
        String headerFont = DropdownTheme.headerFontId();
        float textY = y + (DropdownTheme.PANEL_HEADER_HEIGHT - textMetrics.textHeight(DropdownTheme.HEADER_TEXT_SCALE, headerFont)) * 0.5f;
        ...
        scope.text(headerTitle, textX, textY, DropdownTheme.HEADER_TEXT_SCALE, MD3Theme.TEXT_PRIMARY, headerFont);
