...
public class FriendDropdownPanel extends AbstractDropdownPanel {
    ...
    public FriendDropdownPanel(int panelIndex) {
        super("friend", GravenTranslations.Gui.TAB_FRIEND, "", panelIndex);
    }
    ...
    protected void drawPanelContent(UiTree.Scope scope, UiTextMetrics textMetrics, int mouseX, int mouseY, float visibleHeight) {
        ...
        inputField.draw(scope, textMetrics, fieldX, fieldY, fieldW, FIELD_HEIGHT, mouseX, mouseY, GravenTranslations.Gui.FRIEND_INPUT_PLACEHOLDER.getTranslatedName(), DropdownTheme.SETTING_TEXT_SCALE);
        ...
        if (friends.isEmpty()) {
            scope.text(GravenTranslations.Gui.FRIEND_EMPTY.getTranslatedName(), x + PADDING, rowY + 4.0f, 0.55f, MD3Theme.TEXT_MUTED);
            return;
        }
