package tech.hakuri.graven.gui.panel.component;
...
public class PanelElements {
    ...
    public static void buildSwitch(UiTree.Scope scope, UiRect rect, float toggleProgress, float hoverProgress) {
        scope.toggle(rect, unitProgress(toggleProgress), unitProgress(hoverProgress));
    }

    /** Lumin Switch 只接受闭区间 [0, 1]，回弹动画的越界值必须在 UI 边界收敛。 */
    public static float unitProgress(float value) {
    ...
    public static FilledFieldColors buildFilledField(UiTree.Scope scope, UiRect bounds, boolean focused, float hoverProgress) {
        ...
        scope.input(bounds, focused, hoverProgress,
                6.0f, null, 0.0f, GravenUiTheme.lumin(new Color(0, 0, 0, 0)),
                null, null,
                null, 0.0f, null);
    ...
    public static void buildAssistChip(UiTree.Scope scope, UiTextMetrics textRenderer, UiRect bounds,
        ...
        scope.chip(bounds, label, textScale, GravenUiTheme.lumin(background), GravenUiTheme.lumin(foreground),
    ...
    public static void buildIconButton(UiTree.Scope scope, UiTextMetrics textRenderer, UiRect bounds,
        ...
        scope.iconButton(bounds, label, scale, GravenUiTheme.lumin(tone), hoverProgress);
    }
