...
import tech.hakuri.graven.utils.world.BlockRegistryUtils;
...
public class RegistryListSelectPopup<T> implements PanelPopupHost.Popup {
        ...
        UiTree tree = UiTree.build(scope -> {
            ...
            scope.pushAbsolute(animatedBounds, popup -> {
                popup.popupCard(animatedBounds.atOrigin(), MD3Theme.CARD_RADIUS, MD3Theme.POPUP_SHADOW_BLUR,
                        GravenUiTheme.lumin(MD3Theme.withAlpha(MD3Theme.SHADOW, (int) (MD3Theme.POPUP_SHADOW_ALPHA * progress))),
                ...
                String summary = setting.getValue().size() + GravenTranslations.Gui.LIST_SELECTED.getTranslatedName();
                ...
                popup.input(searchBounds.relativeTo(animatedBounds), true, 1.0f, 8.0f,
                        query.isEmpty() ? GravenTranslations.Gui.LIST_SEARCH.getTranslatedName() : query, 0.54f,
                        GravenUiTheme.lumin(query.isEmpty() ? MD3Theme.TEXT_MUTED : MD3Theme.TEXT_PRIMARY),
                        query.length(), GravenUiTheme.lumin(MD3Theme.PRIMARY), null, 0.0f, null);
                ...
                if (!categories.isEmpty()) {
                    ...
                    for (int ci = -1; ci < categories.size(); ci++) {
                        String catName = ci < 0 ? GravenTranslations.Gui.LIST_ALL.getTranslatedName() : categories.get(ci).name();
                ...
                popup.text(GravenTranslations.Gui.LIST_AVAILABLE.getTranslatedName(), leftX - animatedBounds.x() + 4.0f, headerTextY - animatedBounds.y(), 0.50f, MD3Theme.TEXT_SECONDARY);
                popup.text(GravenTranslations.Gui.LIST_SELECTED_HEADER.getTranslatedName(), rightX - animatedBounds.x() + 4.0f, headerTextY - animatedBounds.y(), 0.50f, MD3Theme.TEXT_SECONDARY);
    ...
    private int toMinecraftGuiXInt(float gravenX) {
        return (int) Math.round(toMinecraftGuiX(gravenX));
    }

    private int toMinecraftGuiYInt(float gravenY) {
        return (int) Math.round(toMinecraftGuiY(gravenY));
    }
