...
public class MainDropdownPanel extends AbstractDropdownPanel {
    ...
    public MainDropdownPanel(int panelIndex, Consumer<String> togglePanel, BooleanSupplier anySubPanelVisible, PanelVisibleResolver panelVisibleResolver) {
        ...
        this.width = DropdownTheme.isOpal() ? 150.0f : 160.0f;
        ...
        add(GravenTranslations.Gui.TAB_FRIEND::getTranslatedName, IconChars.PEOPLE, "friend", togglePanel, panelVisibleResolver);
        add(GravenTranslations.Gui.TAB_CONFIG::getTranslatedName, IconChars.SETTINGS, "config", togglePanel, panelVisibleResolver);
        add(GravenTranslations.Gui.TAB_ADDON::getTranslatedName, IconChars.ADD, "addon", togglePanel, panelVisibleResolver);
        entries.add(new Entry(GravenTranslations.Gui.DROPDOWN_COLLAPSE_ALL::getTranslatedName, IconChars.CLOSE, "__collapse_all__", togglePanel, anySubPanelVisible));
    }
    ...
    public void drawBackground(UiTree.Scope scope, UiTextMetrics textMetrics) {
        ...
        String headerFont = DropdownTheme.headerFontId();
        float versionX = x + textMetrics.textWidth(getTitle(), DropdownTheme.HEADER_TEXT_SCALE, headerFont) + 12.0f;
        ...
        if (!version.isEmpty()) {
            float nameY = y + (DropdownTheme.PANEL_HEADER_HEIGHT - textMetrics.textHeight(DropdownTheme.HEADER_TEXT_SCALE, headerFont)) * 0.5f;
            float versionY = nameY + textMetrics.textHeight(DropdownTheme.HEADER_TEXT_SCALE, headerFont) - textMetrics.textHeight(versionScale, null);
