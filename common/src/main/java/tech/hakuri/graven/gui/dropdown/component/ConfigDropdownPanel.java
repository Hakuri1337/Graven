...
public class ConfigDropdownPanel extends AbstractDropdownPanel {
    ...
    public ConfigDropdownPanel(int panelIndex) {
        super("config", GravenTranslations.Gui.TAB_CONFIG, "", panelIndex);
    }
    ...
    protected void drawPanelContent(UiTree.Scope scope, UiTextMetrics textMetrics, int mouseX, int mouseY, float visibleHeight) {
        ...
        String[] actions = {
                GravenTranslations.Gui.CONFIG_ACTION_SAVE_AS.getTranslatedName(),
                GravenTranslations.Gui.CONFIG_ACTION_RELOAD.getTranslatedName(),
                GravenTranslations.Gui.CONFIG_ACTION_EXPORT.getTranslatedName(),
                GravenTranslations.Gui.CONFIG_ACTION_IMPORT.getTranslatedName(),
                GravenTranslations.Gui.CONFIG_ACTION_NEW.getTranslatedName(),
                GravenTranslations.Gui.CONFIG_ACTION_OPEN_FOLDER.getTranslatedName()
        };
        ...
        if (configs.isEmpty()) {
            ...
            scope.text(GravenTranslations.Gui.CONFIG_EMPTY.getTranslatedName(), contentX, getCenteredTextY(textMetrics, currentY, ROW_HEIGHT, emptyScale), emptyScale, MD3Theme.TEXT_MUTED);
            return;
        }
        ...
        for (String name : configsForFrame()) {
            ...
            if (isHovered(mouseX, mouseY, deleteX, currentY + 3.0f, 16.0f, 16.0f)) {
                try {
                    ...
                    status = GravenTranslations.Gui.DROPDOWN_STATUS_DELETED.getTranslatedName() + " " + name;
                } catch (Exception e) {
                    status = errorText(e);
                }
                return true;
            ...
            if (isHovered(mouseX, mouseY, contentX, currentY, contentW, ROW_HEIGHT)) {
                try {
                    ...
                    status = GravenTranslations.Gui.DROPDOWN_STATUS_SWITCHED.getTranslatedName() + " " + name;
                } catch (Exception e) {
                    status = errorText(e);
                }
                return true;
            ...
            switch (index) {
                case 0 -> {
                    if (!value.isEmpty()) {
                        ...
                        inputField.setCursorToEnd();
                        status = GravenTranslations.Gui.DROPDOWN_STATUS_SAVED.getTranslatedName() + " " + saved;
                    }
                }
                case 1 -> {
                    ConfigHolder.INSTANCE.reloadOrThrow();
                    status = GravenTranslations.Gui.DROPDOWN_STATUS_RELOADED.getTranslatedName();
                }
                case 2 -> {
                    status = GravenTranslations.Gui.DROPDOWN_STATUS_EXPORTED.getTranslatedName() + " " + ConfigHolder.INSTANCE.exportActiveConfigToZip(value).getFileName();
                }
                case 3 -> {
                    if (!value.isEmpty()) {
                        ...
                        inputField.setCursorToEnd();
                        status = GravenTranslations.Gui.DROPDOWN_STATUS_IMPORTED.getTranslatedName() + " " + imported;
                    }
                }
                case 4 -> {
                    if (!value.isEmpty()) {
                        ...
                        inputField.setCursorToEnd();
                        status = GravenTranslations.Gui.DROPDOWN_STATUS_CREATED.getTranslatedName() + " " + created;
                    }
                }
                case 5 ->
                        status = GravenTranslations.Gui.CONFIG_ACTION_OPEN_FOLDER.getTranslatedName() + " " + ConfigFolderOpener.openConfigFolder();
                default -> {
                }
            }
        } catch (Exception e) {
    ...
    private String errorText(Exception e) {
        ...
        return GravenTranslations.Gui.CONFIG_ERROR_TITLE.getTranslatedName() + ": " + (message == null || message.isBlank() ? e.getClass().getSimpleName() : message);
    }
