...
public class ClientSettingTextField {
    ...
    public void buildUi(UiTree.Scope scope, UiRect bounds, int mouseX, int mouseY,
        ...
        scope.input(bounds, focused, hovered ? 0.6f : 0.0f,
                focusProgress, GravenUiTheme.lumin(MD3Theme.PRIMARY), 1.0f,
                textInset, display, textScale, GravenUiTheme.lumin(textColor),
                null, null,
                focused ? Math.min(cursor, text.length()) : null,
                focused ? GravenUiTheme.lumin(MD3Theme.TEXT_PRIMARY) : null,
                ...
                focused && trailingHint != null && !trailingHint.isBlank() && !text.isEmpty()
                        ? GravenUiTheme.lumin(MD3Theme.TEXT_MUTED) : null);
