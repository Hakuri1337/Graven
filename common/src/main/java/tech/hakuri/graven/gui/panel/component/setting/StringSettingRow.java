...
public class StringSettingRow extends SettingRow<StringSetting> {
    ...
    public void buildUi(UiTree.Scope scope, GuiGraphicsExtractor guiGraphics, UiTextMetrics textRenderer, UiRect bounds, float hoverProgress, int mouseX, int mouseY, float partialTick) {
        ...
        scope.input(fieldBounds.relativeTo(bounds), focused, fieldHover,
                0.0f, GravenUiTheme.lumin(new Color(0, 0, 0, 0)), 0.0f,
                slice.textX() - fieldBounds.x(), slice.text(), FIELD_SCALE,
                GravenUiTheme.lumin(MD3Theme.filledFieldContent(focused)),
                selection, selection == null ? null : GravenUiTheme.lumin(MD3Theme.withAlpha(MD3Theme.filledFieldIndicator(focused, fieldHover), 90)),
                focused ? slice.caretIndex() : null,
                focused ? GravenUiTheme.lumin(MD3Theme.filledFieldCaret(focused)) : null,
                null, 0.0f, null);
        if (focused) {
