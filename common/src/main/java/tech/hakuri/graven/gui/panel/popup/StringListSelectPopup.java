    ...
    public void extractGui(GuiGraphicsExtractor guiGraphics, UiRenderBatch renderBatch, int mouseX, int mouseY, float partialTick) {
        ...
        UiTree tree = UiTree.build(scope -> {
            ...
            scope.pushAbsolute(animatedBounds, popup -> {
                ...
                String summary = setting.getValue().size() + GravenTranslations.Gui.LIST_ENTRIES.getTranslatedName();
                ...
                String placeholder = input.isEmpty() ? GravenTranslations.Gui.LIST_TYPE_TO_ADD.getTranslatedName() : input;
                popup.input(inputBounds.relativeTo(animatedBounds), true, 1.0f, 8.0f,
                        placeholder, 0.54f,
                        GravenUiTheme.lumin(input.isEmpty() ? MD3Theme.TEXT_MUTED : MD3Theme.TEXT_PRIMARY),
                        input.length(), GravenUiTheme.lumin(MD3Theme.PRIMARY), null, 0.0f, null);
