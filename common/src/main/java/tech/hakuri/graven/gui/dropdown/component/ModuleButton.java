...
import tech.hakuri.graven.gui.dropdown.widget.*;
...
public class ModuleButton extends Component {
    ...
    public void draw(UiTree.Scope scope, UiTextMetrics textMetrics, int mouseX, int mouseY) {
        ...
        Color disabled = DropdownTheme.moduleDisabled(hover);
        Color bgStart = MD3Theme.lerp(disabled, DropdownTheme.moduleEnabled(hover), toggle);
        Color bgEnd = MD3Theme.lerp(disabled, DropdownTheme.moduleEnabledAlternate(hover), toggle);
        if (DropdownTheme.isOpal()) {
            scope.roundRectHorizontalGradient(2.0f, 0.0f, width - 4.0f, DropdownTheme.MODULE_HEIGHT, 1.0f,
                    GravenUiTheme.lumin(bgStart), GravenUiTheme.lumin(bgEnd));
        } else {
            scope.rect(2.0f, 0.0f, width - 4.0f, DropdownTheme.MODULE_HEIGHT, bgStart);
        }
        ...
        float controlsX = width - DropdownTheme.MODULE_PADDING_X - DropdownTheme.KEYBIND_WIDTH - 4.0f - 18.0f;
        String moduleName = trimToWidth(module.getTranslatedName(), textMetrics,
                Math.max(8.0f, controlsX - leftX - 3.0f));
        scope.text(moduleName, leftX, textY, DropdownTheme.MODULE_TEXT_SCALE, textColor);
    ...
    private void drawAddonInfo(UiTree.Scope scope, UiTextMetrics textMetrics, float infoY) {
        ...
        String addonLabel = GravenTranslations.Module.FROM.getTranslatedName() + " " + getAddonLabel();
    ...
    private void drawKeybindButton(UiTree.Scope scope, UiTextMetrics textMetrics, int mouseX, int mouseY, float toggle) {
        ...
        if (listeningKeybind) {
            ...
            outline = DropdownTheme.controlOutline(1.0f);
        } else {
            Color idleSurface = DropdownTheme.keybindSurface(false);
            Color activeSurface = DropdownTheme.isOpal() ? new Color(36, 153, 203, 220) : MD3Theme.PRIMARY;
            surface = MD3Theme.lerp(idleSurface, activeSurface, toggle);
            surface = MD3Theme.lerp(surface, MD3Theme.TEXT_PRIMARY, kbHover * 0.08f);
            outline = DropdownTheme.controlOutline(Math.max(toggle * 0.65f, kbHover));
        }
    ...
    private String trimToWidth(String value, UiTextMetrics textMetrics, float maxWidth) {
        if (textMetrics.textWidth(value, DropdownTheme.MODULE_TEXT_SCALE, null) <= maxWidth) return value;
        String ellipsis = "...";
        for (int length = value.length() - 1; length >= 0; length--) {
            String candidate = value.substring(0, length) + ellipsis;
            if (textMetrics.textWidth(candidate, DropdownTheme.MODULE_TEXT_SCALE, null) <= maxWidth) {
                return candidate;
            }
        }
        return ellipsis;
    ...
    private void drawHiddenButton(UiTree.Scope scope, UiTextMetrics textMetrics, int mouseX, int mouseY) {
        ...
        if (!module.isHidden()) {
            Color hiddenSurface = DropdownTheme.isOpal()
                    ? MD3Theme.lerp(new Color(30, 30, 38, 225), new Color(45, 191, 254, 145), hovered ? 1.0f : 0.0f)
                    : MD3Theme.lerp(MD3Theme.SECONDARY_CONTAINER, MD3Theme.SECONDARY, hovered ? 0.12f : 0.0f);
            scope.roundRect(btnX, btnY, btnW, btnH, DropdownTheme.KEYBIND_RADIUS, hiddenSurface);
            String icon = IconChars.VISIBILITY;
            float scale = 0.58f;
            String iconFont = "graven-icons";
            float iconW = textMetrics.textWidth(icon, scale, iconFont);
            float iconH = textMetrics.textHeight(scale, iconFont);
            scope.text(icon, btnX + (btnW - iconW) * 0.5f, btnY + (btnH - iconH) * 0.5f - 1.0f,
                    scale, DropdownTheme.isOpal() ? Color.WHITE : MD3Theme.ON_SECONDARY_CONTAINER, iconFont);
        }
        if (hovered) {
            String hint = module.isHidden() ? GravenTranslations.Module.HIDDEN.getTranslatedName() : GravenTranslations.Module.VISIBLE.getTranslatedName();
