...
import tech.hakuri.graven.gui.panel.popup.ConfirmActionPopup;
import tech.hakuri.graven.gui.panel.popup.MessagePopup;
...
import tech.hakuri.graven.utils.client.ConfigFolderOpener;
...
public class ConfigClientSettingTab implements ClientSettingTabView {
    ...
    public void render(GuiGraphicsExtractor guiGraphics, UiRenderBatch renderBatch, UiRect bounds, int mouseX, int mouseY, float partialTick) {
        ...
        UiTree tree = UiTree.build(scope -> {
            inputField.buildUi(scope, getInputFieldBounds(inputSection), mouseX, mouseY, textRenderer,
                    GravenTranslations.Gui.CONFIG_INPUT_PLACEHOLDER.getTranslatedName(), FIELD_SCALE, null);
            ...
            scope.viewport(contentBuffer, listViewport, state.getConfigScroll(), maxScroll, contentHeight, mouseX, mouseY, content -> {
                ...
                if (configs.isEmpty()) {
                    ...
                    String hint = GravenTranslations.Gui.CONFIG_EMPTY.getTranslatedName();
    ...
    private void buildConfigRow(UiTree.Scope scope, String configName, String activeConfig, UiRect rowBounds, UiRect deleteBounds, float hover, float deleteHover) {
        ...
        String subtitle = active ? GravenTranslations.Gui.CONFIG_CURRENT.getTranslatedName() : GravenTranslations.Gui.CONFIG_SWITCH_HINT.getTranslatedName();
        ...
        if (active) {
            String chipText = GravenTranslations.Gui.CONFIG_CURRENT.getTranslatedName();
    ...
    private void trySaveAs() {
        ...
        try {
            ...
            openErrorPopup(GravenTranslations.Gui.CONFIG_ERROR_SAVE::getTranslatedName, exception);
        }
    }
    ...
    private void tryReload() {
        try {
            ...
            openErrorPopup(GravenTranslations.Gui.CONFIG_ERROR_RELOAD::getTranslatedName, exception);
        }
    }
    ...
    private void tryExport() {
        try {
            ...
            openErrorPopup(GravenTranslations.Gui.CONFIG_ERROR_EXPORT::getTranslatedName, exception);
        }
    }
    ...
    private void tryImport() {
        ...
        try {
            ...
            openErrorPopup(GravenTranslations.Gui.CONFIG_ERROR_IMPORT::getTranslatedName, exception);
        }
    }
    ...
    private void tryOpenFolder() {
        try {
            ...
            openErrorPopup(GravenTranslations.Gui.CONFIG_ERROR_OPEN_FOLDER::getTranslatedName, exception);
        }
    }
    ...
    private void trySwitchConfig(String configName) {
        ...
        try {
            ...
            openErrorPopup(GravenTranslations.Gui.CONFIG_ERROR_SWITCH::getTranslatedName, exception);
        }
    }
    ...
    private void tryDeleteConfig(String configName) {
        try {
            if (!ConfigHolder.INSTANCE.deleteConfig(configName)) {
                openErrorPopup(GravenTranslations.Gui.CONFIG_ERROR_DELETE::getTranslatedName, GravenTranslations.Gui.CONFIG_ERROR_DELETE_LAST.getTranslatedName());
                return;
            }
            ...
            openErrorPopup(GravenTranslations.Gui.CONFIG_ERROR_DELETE::getTranslatedName, exception);
        }
    }
    ...
    private void openDeleteConfirmation(String configName) {
        ...
        popupHost.open(new ConfirmActionPopup(
                popupBounds,
                GravenTranslations.Gui.CONFIG_DELETE_CONFIRM_TITLE::getTranslatedName,
                GravenTranslations.Gui.CONFIG_DELETE_CONFIRM_MESSAGE::getTranslatedName,
                trimToWidth(configName, 0.60f, popupWidth - 24.0f),
                GravenTranslations.Gui.CONFIG_DELETE_CONFIRM_CONFIRM::getTranslatedName,
                GravenTranslations.Gui.CONFIG_DELETE_CONFIRM_CANCEL::getTranslatedName,
                () -> {
    ...
    private void openErrorPopup(Supplier<String> actionMessageSupplier, String detail) {
        ...
        popupHost.open(new MessagePopup(
                ...
                GravenTranslations.Gui.CONFIG_ERROR_TITLE::getTranslatedName,
                actionMessageSupplier,
                trimToWidth(detail, 0.52f, popupWidth - 24.0f),
                GravenTranslations.Gui.CONFIG_ERROR_OK::getTranslatedName
        ));
    }
    ...
    private void openExportSuccessPopup(Path exported) {
        ...
        popupHost.open(new MessagePopup(
                popupHost.getCenteredBounds(popupWidth, popupHeight),
                GravenTranslations.Gui.CONFIG_EXPORT_SUCCESS_TITLE::getTranslatedName,
                GravenTranslations.Gui.CONFIG_EXPORT_SUCCESS_MESSAGE::getTranslatedName,
                trimToWidth(detail, 0.52f, popupWidth - 24.0f),
                GravenTranslations.Gui.CONFIG_ERROR_OK::getTranslatedName
        ));
    }
    ...
    private List<ActionButton> getActionButtons(UiRect inputBounds) {
        ...
        return List.of(
                new ActionButton(ActionButtonType.SAVE_AS, GravenTranslations.Gui.CONFIG_ACTION_SAVE_AS.getTranslatedName(), new UiRect(inputBounds.x(), y, width, BUTTON_HEIGHT)),
                new ActionButton(ActionButtonType.RELOAD, GravenTranslations.Gui.CONFIG_ACTION_RELOAD.getTranslatedName(), new UiRect(inputBounds.x() + width + gap, y, width, BUTTON_HEIGHT)),
                new ActionButton(ActionButtonType.EXPORT, GravenTranslations.Gui.CONFIG_ACTION_EXPORT.getTranslatedName(), new UiRect(inputBounds.x() + (width + gap) * 2.0f, y, width, BUTTON_HEIGHT)),
                new ActionButton(ActionButtonType.IMPORT, GravenTranslations.Gui.CONFIG_ACTION_IMPORT.getTranslatedName(), new UiRect(inputBounds.x() + (width + gap) * 3.0f, y, width, BUTTON_HEIGHT)),
                new ActionButton(ActionButtonType.NEW, GravenTranslations.Gui.CONFIG_ACTION_NEW.getTranslatedName(), new UiRect(inputBounds.x(), secondRowY, secondRowWidth, BUTTON_HEIGHT)),
                new ActionButton(ActionButtonType.OPEN_FOLDER, GravenTranslations.Gui.CONFIG_ACTION_OPEN_FOLDER.getTranslatedName(), new UiRect(inputBounds.x() + secondRowWidth + gap, secondRowY, secondRowWidth, BUTTON_HEIGHT))
        );
    }
