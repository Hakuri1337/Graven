...
MusicPlayer 模块实现记录见 [`development/music-player-implementation.md`](development/music-player-implementation.md)。
...
| 下游 Asaka GrimVelocity 状态机、完整依赖与严格迁移 | [Asaka GrimVelocity 完整重迁移](development/asaka-velocity-grim-migration.md) |
...
| Remix TpAura 系列、特色模块、CubeCraft Disabler 与上游 AutoGapple 严格迁移 | [Remix 特色模块严格迁移](development/remix-teleport-modules-migration.md) |
| Rise New Universal Keep Sprint 在 KillAura 中的 Heypixel KeepSprint 严格迁移 | [Rise Heypixel KeepSprint 迁移](development/rise-heypixel-keepsprint-migration.md) |
...
| MainMenu `New` 视频背景、JCodec 解码与动态纹理生命周期 | [MainMenu 视频背景](development/main-menu-video-background.md) |
| Target HUD 动态皮肤资源检查与回退 | [Target HUD 纹理回退](development/target-hud-texture-fallback.md) |
| wyapi.toubiec.cn 前端与 nextmusic.toubiec.cn 后端 API 逆向记录 | [wyapi API 逆向记录](development/wyapi-api-reverse-analysis.md) |
