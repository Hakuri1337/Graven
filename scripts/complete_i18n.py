#!/usr/bin/env python3
"""用 runClient 生成的嵌套 graven-empty-i18n.json 同步全部或单个 owner 的语言文件。"""
...
DEFAULT_I18N_DIR = PROJECT_ROOT / "common" / "src" / "main" / "resources" / "assets" / "graven" / "i18n"
EMPTY_I18N_FILE = "graven-empty-i18n.json"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="根据 graven-empty-i18n.json 为指定 i18n JSON 文件补全、排序并删除多余翻译 key。",
    )
    parser.add_argument(
        "--source",
    ...
    parser.add_argument(
        "--empty-i18n",
        type=Path,
        help="自定义 graven-empty-i18n.json 路径。设置后等同于 --source custom。",
    )
    parser.add_argument(
        "--target",
        type=Path,
    ...
    parser.add_argument(
        ...
        "--match-owner",
        dest="owner",
        help="仅同步一个 owner；本体使用 graven，Addon 使用其 addonId。未指定时同步全部。",
    )
    parser.add_argument(
        "--missing-value",
        default="",
    ...
    sort_group.add_argument(
        ...
        dest="sort",
        action="store_true",
        help="按 graven-empty-i18n.json 中的 key 顺序排序目标文件。",
    )
    sort_group.add_argument(
        "--no-sort",
        dest="sort",
...
def choose_empty_i18n_path(args: argparse.Namespace) -> Path:
    ...
    source = args.source
    if source is None:
        print("请选择 graven-empty-i18n.json 来源：")
        print("  1. fabric   -> fabric/runs/client/graven-empty-i18n.json")
        print("  2. neoforge -> neoforge/runs/client/graven-empty-i18n.json")
    ...
    if source == "custom":
        return resolve_path(Path(prompt("请输入 graven-empty-i18n.json 路径")))
...
def choose_sort_enabled(args: argparse.Namespace) -> bool:
    if args.sort is not None:
        return args.sort
    if sys.stdin.isatty():
        return prompt_bool("是否按 graven-empty-i18n.json 的 key 顺序排序", True)
    return False
